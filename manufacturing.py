...
global_file.windPower = pd.read_csv("dataPower.csv")['windPower'].tolist()
...
for p in range(4):
    for td in range(4):
        ...
        for reps in range(replications):
            ...
            peak_power = np.array([0]*24*2*30*2)  # 24 hours, 30 minute one hour, and 30 days, 2 months
            ...
            for t in range(96,96+48*5+48*30):
                ...
                if (t/2)%24 < global_file.shift_start or (t/2)%24 > global_file.shift_end:
                    ...
                    A += global_file.windPower[t/2]*wind_multiply
                    A = min(AM,A)
                ...
                # inside the shift, the optimization decisions are made
                    ...
                    # Prediction model  
                        ...
                        prediction = function_file.energy(t/2,t_dash,wind_multiply)
                        ...
                        for j in range(global_file.M):
                            ...
                            if machines[j].on_time + global_file.stay_time < t and global_file.site[(t/2)%24]-global_file.windPower[t/2]*wind_multiply- min_t+machines[j].idle_power/(machines[j].on_power+machines[j].off_power) > delay_threshold1 and global_file.site[(t/2)%24]-global_file.windPower[t/2]*wind_multiply > min_t:
                        ...
                        reneP = global_file.windPower[t/2]*wind_multiply
                        ...
                        Y = optimal.addVar(0,min(A-YM,YM),0,GRB.CONTINUOUS,'Y')
                            ...
                            if t >= machines[j].scheduled_time and t >= machines[j].off_time and len(machines[j].queue) > 0:
                                ...
                                for q in range(len(machines[j].queue)):
                                    ...
                                    for l in range(t_dash*2+1):
                                        ...
                                        objective.addTerms(-alpha_d*D,temp_x[-1])
                        ...
                        for l in range(t_dash*2+1):
                            ...
                            if l == 0:
                                ...
                                optimal.addConstr(supply_demand,GRB.LESS_EQUAL,0)
                            else:
                        ...
                        if sum(y_active) > 0: 
                            ...
                            #print t,P,D,dd_reduction, dd_reduction_optimal, optimal.ObjVal,sum([L[l].X for l in range(7)])
                            
                        # if no machine is turned on, then just update the battery and D,P
                        else:
                            A += global_file.windPower[t/2]*wind_multiply
                            A = min(A,AM)
                            peak_power[t] -= global_file.windPower[t/2]*wind_multiply
